...
export default function ShiftsView({ session }: { session: AppSession }) {
  ...
  const handleSave = async () => {
    if (!form.employee_id || !form.shift_date) {
      setError('Empleado y fecha son obligatorios')
      return
    }
  ...
  const f = (v: string | number, k: string) => {
    setForm((p) => ({ ...p, [k]: v }))
  }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Btn variant="secondary" onClick={() => setWeekOffset((w) => w - 1)}>
            ← Anterior
          </Btn>
          <Btn variant="secondary" onClick={() => setWeekOffset(0)}>
            Hoy
          </Btn>
          <Btn variant="secondary" onClick={() => setWeekOffset((w) => w + 1)}>
            Siguiente →
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 860 }}>
              <thead>
                <tr style={{ background: C.hi }}>
                  <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 11, color: C.muted, fontWeight: 700, width: 160, borderBottom: `1px solid ${C.border}` }}>
                    EMPLEADO
                  </th>
                  ...
                  {weekDates.map((d, i) => (
                    <th
                      ...
                      style={{
                        ...
                        textAlign: 'center',
                        fontSize: 11,
                        color: d === new Date().toISOString().split('T')[0] ? C.accent : C.muted,
                        fontWeight: 700,
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: 10, color: C.muted }}>
                              {empShifts.reduce((a, s) => a + s.total_hours, 0).toFixed(1)}h / sem
                            </div>
                          </div>
                        </div>
                      </td>
                        ...
                        return (
                          <td key={date} style={{ padding: 6, textAlign: 'center', verticalAlign: 'middle' }}>
                            {shift ? (
                              ...
                              >
                                <div style={{ fontSize: 11, fontFamily: 'monospace', color: C.accent, fontWeight: 700 }}>
                                  {shift.start_time}
                                </div>
                              ...
                              <div style={{ height: 70, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.border }}>
                                {isManager ? (
                                  <button
                                    type="button"
                                    onClick={() => {
                                      setForm((prev) => ({ ...prev, employee_id: emp.id, shift_date: date }))
                                      setEditing(null)
                                      setShowModal(true)
                                    }}
                                    style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: error ? 12 : 0 }}>
          {isManager && (
            <Select
              ...
              onChange={(e) => f(e.target.value, 'employee_id')}
          ...
          <Select
            ...
            onChange={(e) => f(e.target.value, 'department_id')}
          ...
          <Select
            label="Estado"
            value={form.status}
            onChange={(e) => f(e.target.value as ShiftStatus, 'status')}
            options={STATUS_OPTIONS.map((s) => ({ value: s, label: s }))}
          />

          <Input label="Notas" value={form.notes} onChange={(e) => f(e.target.value, 'notes')} placeholder="Opcional..." style={{ gridColumn: 'span 2' }} />
        </div>
